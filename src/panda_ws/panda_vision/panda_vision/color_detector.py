...
# --- FIX 1: PARCHE DE NUMPY (Para evitar el error 'no attribute float') ---
import numpy as np
# Si la versión de Numpy es nueva y no tiene 'float', se lo inyectamos manualmente
...
# --- FIX 2: IMPORTAR LIBRERÍAS QoS (Para que conecte con Gazebo) ---
...
class ColorDetector(Node):
    def __init__(self):
        ...
        # --- FIX 3: CONFIGURACIÓN QoS (Best Effort) ---
        # Esto es OBLIGATORIO para ver la cámara de Gazebo
        qos_policy = QoSProfile(
            reliability=ReliabilityPolicy.BEST_EFFORT,
            depth=10
        )

        # Subscriber actualizado con QoS
        self.image_sub = self.create_subscription(
            Image, 
            '/camera/image_raw', 
            self.image_callback, 
            qos_policy) # <--- Usamos qos_policy aquí
        # ----------------------------------------------
    ...
    def image_callback(self, msg):
        try:
            ...
            # Debug: Confirmar que llega imagen
